"""
option_chain_fixed.py - FULLY FIXED VERSION
====================================
Institutional-grade NSE India Options Chain Dashboard
✅ UPDATED API ENDPOINTS (NSE, Alternative Sources)
✅ FALLBACK MECHANISMS (yfinance, manual data input)
...
# ══════════════════════════════════════════════════════════════════════════
# CONSTANTS - UPDATED API ENDPOINTS
# ══════════════════════════════════════════════════════════════════════════

NSE_BASE_URL = "https://www.nseindia.com"

# ✅ UPDATED ENDPOINTS (Multiple fallback options)
NSE_API_ENDPOINTS = {
    "option_chain_v1": f"{NSE_BASE_URL}/api/option-chain-indices",  # Legacy (may be down)
    "option_chain_v2": f"{NSE_BASE_URL}/api/option-chain",  # Alternative path
    "live_data": f"{NSE_BASE_URL}/api/equity-stockIndices",  # Spot prices
    "quote": f"{NSE_BASE_URL}/api/quote-equity",  # Quote endpoint
}
...
# ✅ IMPROVED HEADERS (Mimic real browser more closely)
_NSE_HEADERS = {
    "User-Agent": (
...
# ══════════════════════════════════════════════════════════════════════════
# HTTP SESSION WITH IMPROVED RETRY LOGIC
# ══════════════════════════════════════════════════════════════════════════

def _build_retrying_session() -> requests.Session:
    """Build session with exponential backoff and better retry config."""
...
def _warm_up_session(session: requests.Session) -> bool:
    """Warm up session with initial requests to NSE."""
    try:
...
def fetch_json_with_retry(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if resp.status_code == 429:
                ...
                last_error = f"Rate limited (429) - wait {wait_time}s on attempt {attempt}/{max_retries}"
            ...
            if resp.status_code in (401, 403):
                last_error = f"HTTP {resp.status_code} (auth/forbidden) on attempt {attempt}/{max_retries}"
            ...
            if resp.status_code >= 500:
                last_error = f"HTTP {resp.status_code} (server error) on attempt {attempt}/{max_retries}"
                logger.warning(last_error)
                time.sleep(RETRY_BACKOFF_SECONDS * attempt)
                continue
            
            # Handle 404 - likely endpoint changed
    ...
    logger.error(f"❌ fetch_json_with_retry exhausted retries for {endpoint_name}: {last_error}")
    return None, last_error


# ══════════════════════════════════════════════════════════════════════════
# DATA FETCH WITH MULTIPLE ENDPOINT STRATEGIES
# ══════════════════════════════════════════════════════════════════════════
...
def fetch_option_chain_raw(symbol: str, is_index: bool, is_commodity: bool = False) -> dict:
    """Fetch option chain with multiple endpoint fallbacks."""
    ...
    if is_index or is_commodity:
        # For indices/commodities, try both v1 and v2 endpoints
        endpoints_to_try = [
    ...
    for url, endpoint_name in endpoints_to_try:
        ...
        if isinstance(records, dict) and records.get("data"):
            ...
            last_error = "Response structure invalid (no 'records.data')"
            logger.warning(f"⚠️ Invalid response from {endpoint_name}: {last_error}")
            continue
...
# ══════════════════════════════════════════════════════════════════════════
# DATA PARSING & VALIDATION (unchanged from original)
# ══════════════════════════════════════════════════════════════════════════
    ...
    with st.sidebar:
        ...
        if is_index:
            ...
            if symbol in NSE_UNSUPPORTED_INDICES:
                st.caption(
                    f"ℹ️ {symbol} is BSE-listed — requires FYERS or other broker integration."
                )
        elif is_commodity:
...
def _do_fetch_and_process(cfg: dict, fyers: Any = None) -> Optional[dict]:
    ...
    if not fetch_result.get("ok"):
        st.error(
            ...
            f"**Troubleshooting:**\n"
            f"• NSE API endpoints may have changed\n"
            f"• Try a different index (NIFTY, BANKNIFTY, FINNIFTY, etc.)\n"
            f"• Check NSE website for availability\n"
            f"• NSE may be rate-limiting — wait and retry"
        )
        return None
...
def run_dashboard(fyers: Any = None) -> None:
    ...
    st.markdown("Greeks Engine + Gamma Exposure + IV Rank")
