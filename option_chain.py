...
def _decorate_live_big_movement_rows(
    ...
    for _, row in out.iterrows():
        ...
        baseline_ts = series[0].get("ts") if series and isinstance(series[0], dict) else None
...
def _render_big_movement_scan(
    ...
    if out.empty:
        st.info(
            "Baseline saved. No CE/PE pre-move is building yet. "
            "Click REFRESH NOW / run this scan again after the next live update."
        )
        if errors:
            st.caption("Scan notes: " + " | ".join(errors[:12]))
        return
