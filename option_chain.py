...
def _movement_signal_time_from_history(
    ...
    strike: float,
    side: str = "CE",
    min_score: float = MOVEMENT_SEARCH_MIN_SCORE,
    score_gap: float = 7.0,
) -> Optional[datetime]:
...
def _movement_price_reversal_from_history(
    ...
    if len(prices) >= 2:
        # Use the CURRENT signal episode, not an old session low.
        # For UP movement, reversal is a trailing reference that can move UP
        # as price makes higher lows. It must never reset to an older low.
        recent = prices[:-1][-max(2, int(lookback)):]
        reversal = max(recent) if current_price > recent[-1] else min(recent)
        if current_price > max(recent):
            reversal = recent[-1]
        return (
            float(reversal),
