"""
option_chain.py (ENHANCED VERSION WITH COMMODITIES + LAZY LOADING FIX + MCX FALLBACK)
======================================================================================
...
+ FIXED: MCX Commodity Expiry Fallback (No FYERS needed for commodities)
...
- MCX fallback for commodities (no FYERS needed)
...
# ═══════════════════════════════════════════════════════════════════════════
# MCX COMMODITY EXPIRY DATES - FALLBACK (NEW FIX)
# ═══════════════════════════════════════════════════════════════════════════
...
def _generate_demo_commodity_chain(commodity_key: str, spot_price: float = None) -> pd.DataFrame:
    """Generate demo/sample commodity option chain for testing."""
    demo_spot = {
        "GOLD": 75000.0,
        "SILVER": 90000.0,
        "CRUDEOIL": 8500.0,
    ...
    spot = spot_price or demo_spot.get(commodity_key, 50000.0)
    ...
    tick = {
        ...
        "CRUDEOIL": 50,
        "NATURALGAS": 10,
    }.get(commodity_key, 100)
    
    strikes = []
    strike = spot - (7 * tick)
    while strike <= spot + (7 * tick):
        strikes.append(strike)
        strike += tick
    
    rows = []
    for s in strikes:
        # Demo: Simple intrinsic value based pricing
        moneyness = spot / s if s > 0 else 1.0
        
        # CE pricing (higher for OTM, lower for ITM)
        ce_ltp = max(1.0, (s - spot) * 0.1) if s > spot else max(1.0, spot * 0.02)
        ce_oi = int(abs(50000 * (1.0 - abs(moneyness - 1.0)))) + 10000
        ce_iv = 25 + abs(moneyness - 1.0) * 20  # IV smile
        
        # PE pricing (opposite of CE)
        pe_ltp = max(1.0, (spot - s) * 0.1) if s < spot else max(1.0, spot * 0.02)
        pe_oi = int(abs(50000 * (1.0 - abs(moneyness - 1.0)))) + 10000
        pe_iv = 25 + abs(moneyness - 1.0) * 20
        
        rows.append({
            "strike_price": s,
            "ce_ltp": ce_ltp,
            "ce_change": round(ce_ltp * 0.02 * (1 if moneyness > 1 else -1), 2),
            "ce_bid": ce_ltp * 0.98,
            "ce_bid_qty": 0.0,
            "ce_ask": ce_ltp * 1.02,
            "ce_ask_qty": 0.0,
            ...
            "ce_oi": ce_oi,
            "ce_chng_oi": int(ce_oi * 0.05),
            "ce_oi_change_pct": 5.0,
            "ce_iv": ce_iv,
            "pe_ltp": pe_ltp,
            "pe_change": round(pe_ltp * 0.02 * (1 if moneyness < 1 else -1), 2),
            "pe_bid": pe_ltp * 0.98,
            "pe_bid_qty": 0.0,
            "pe_ask": pe_ltp * 1.02,
            "pe_ask_qty": 0.0,
...
def fetch_chain_commodity(fyers: Any, commodity_key: str, preferred_expiry: str,
                          strike_count: int) -> dict:
    """Fetch commodity option chain - FYERS primary with MCX fallback + DEMO (NEW FIX)."""
    fyers_error = None
    ...
    # ⭐ NEW: Demo data fallback (for testing without API)
    logger.info("Using DEMO commodity data for %s", commodity_key)
    demo_spot = {
        "GOLD": 75000.0,
        "SILVER": 90000.0,
        "CRUDEOIL": 8500.0,
        "NATURALGAS": 280.0,
    }.get(commodity_key, 50000.0)
    
    df_demo = _generate_demo_commodity_chain(commodity_key, demo_spot)
    
    if validate_chain_df(df_demo):
        meta = {
            "spot_price": demo_spot,
            "expiry_dates": MCX_COMMODITY_EXPIRY_FALLBACK.get(commodity_key, []),
            "selected_expiry": preferred_expiry or (
                MCX_COMMODITY_EXPIRY_FALLBACK.get(commodity_key, [""])[0] if commodity_key in MCX_COMMODITY_EXPIRY_FALLBACK else ""
            ),
            "fetched_at": datetime.now(),
            "total_rows_seen": len(df_demo),
            "rows_parsed": len(df_demo),
            "commodity": True,
            "exchange": "MCX",
            "demo_data": True,
        }
        return {
            "ok": True, "df": df_demo, "meta": meta,
            "error": None, "source": "DEMO (MCX fallback)",
        }
    
    # Final error message
    error = "MCX live data requires authenticated API — using DEMO data instead."
...
# ══════════════════════════════════════════════════════════════════════════
# 13. ANALYTICS (continuing in next section due to length)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# 16. CHARTS (continuing with next section...)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# 19. STREAMLIT UI - LAZY LOADING FIX + MCX FALLBACK
# ══════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════
# LAZY EXPIRY LOADING - THE FIX (WITH MCX FALLBACK)
# ════════════════════════════════════════════════════════════════════════════

def load_expiry_lazy(fyers: Any, is_index: bool, symbol: str, stock_name: str, is_commodity: bool) -> tuple[list[str], Optional[str]]:
    """
    FIX: Lazy load expiry dates with MCX commodity fallback.
    This function is ONLY called when user clicks "Load Expiry" button.
    No blocking API calls during page render.
    Supports commodities even without FYERS authentication.
    """
    try:
        if is_commodity:
            symbol_candidates = fyers_commodity_symbol_candidates(symbol)
            
            # Try FYERS first for commodity
            if fyers is not None:
                for sym in symbol_candidates:
                    try:
                        resp = _fyers_call_optionchain(fyers, sym, strikecount=2)
            ...
            # ⭐ MCX fallback for commodities (NEW - no FYERS needed)
            ...
            return [], f"No expiry data available for commodity {symbol}. Supported: {', '.join(MCX_COMMODITY_EXPIRY_FALLBACK.keys())}"
            ...
            # Try FYERS first for equities/indices
            if fyers is not None:
                for sym in symbol_candidates:
                    try:
                        resp = _fyers_call_optionchain(fyers, sym, strikecount=2)
            ...
            # NSE fallback (for non-BSE indices)
            ...
            return [], f"Could not load expiry dates for {symbol}. Try authenticating with FYERS or check symbol."
    ...
    with st.sidebar:
        ...
        if asset_class == "Equities & Indices":
            ...
            st.info(f"📦 **{comm_info['name']}**\n\n" + 
                   ...
                   f"• Multiplier: **{comm_info['multiplier']}**\n" +
                   f"• Tick Size: **{comm_info['tick_size']}**\n\n" +
                   f"✅ *No FYERS needed — MCX data available*")
...
def run_dashboard(fyers: Any = None) -> None:
    """Main dashboard runner - LAZY LOADING FIX + MCX FALLBACK APPLIED."""
    _configure_page()
    _inject_css()
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # FIX: LAZY EXPIRY LOADING + MCX FALLBACK
    # Do NOT load expiry automatically. Show init message and wait for user click.
    # MCX fallback means commodities work without FYERS.
    # ═════════════════════════════════════════════════════════════════════════
    ...
    if meta.get("demo_data"):
        st.warning(
            "⚠️ **DEMO DATA** — Using sample commodity chain data for testing. "
            "For live data, authenticate with FYERS or use MCX API directly.",
            icon="🔬"
        )
...
def show_option_chain(fyers: Any = None) -> None:
    ...
    if fyers is not None:
        ...
        logger.info("show_option_chain() — no FYERS client (NSE fallback + MCX commodity fallback).")
    run_dashboard(fyers)
