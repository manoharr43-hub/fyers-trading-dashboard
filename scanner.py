...
# PAGE CONFIG
st.set_page_config(page_title="NSE AI PRO V16.1", layout="wide", initial_sidebar_state="expanded")
...
if "fo_symbols_loaded" not in st.session_state:
    st.session_state.fo_symbols_loaded = False
...
def load_nse_equity_symbols() -> List[str]:
    ...
    try:
        st.write("📥 Loading NSE symbols...")
        resp = requests.get(FYERS_NSE_CM_SYMBOL_MASTER, timeout=30)
        resp.raise_for_status()
        st.write("✅ Downloaded symbol master")
    ...
    st.write(f"✅ Loaded {len(valid)} valid NSE equity symbols")
        ...
        for url in ["https://public.fyers.in/sym_details/NSE_FO.csv"]:
            try:
                ...
                if r.text and len(r.text) > 100:
                    ...
                    st.write(f"✅ Loaded {len(result)} F&O stocks")
                    logger.info(f"Loaded {len(result)} F&O stocks")
                    return result
            except Exception as e:
                st.warning(f"⚠️ F&O load error: {e}")
                continue
        return []
    except Exception as e:
...
def _safe_history(fyers, params, max_retries=3):
    ...
    return None, f"{symbol}: Failed to fetch data"
...
def fetch_stock_data(fyers, symbol, resolution="5", lookback_days=30):
    ...
    try:
        ...
        logger.error(f"Data processing error {symbol}: {e}")
        return None
...
def main():
    ...
    st.info("✅ Signal times fixed | ✅ Pandas 2.0 compatible | ✅ Production ready")
    
    # LOAD SYMBOLS
    st.header("📊 Loading Symbols")
    ...
    with col1:
        if st.button("🔄 Load NSE Symbols", use_container_width=True):
            ...
            st.success("✅ Symbols loaded!")
    
    with col2:
        if st.button("🔄 Load F&O Stocks", use_container_width=True):
            st.session_state.fo_symbols = load_fo_stocks()
            st.session_state.fo_symbols_loaded = True
            st.success("✅ F&O stocks loaded!")
    
    # DISPLAY LOADED DATA
    if st.session_state.symbols_loaded:
        st.metric("NSE Symbols", len(st.session_state.all_symbols))
    
    if st.session_state.fo_symbols_loaded:
        st.metric("F&O Stocks", len(st.session_state.fo_symbols))
    
    # SCANNER TABS
    tabs = st.tabs(["📈 Stock Analysis", "⚙️ Settings", "📊 Dashboard"])
    
    with tabs[0]:
        st.header("📈 Individual Stock Analysis")
        
        if not st.session_state.all_symbols:
            st.warning("⚠️ Click 'Load NSE Symbols' first!")
        else:
            # Stock selector
                ...
                if st.button("📊 Analyze", use_container_width=True, key="analyze_btn"):
                    ...
                    try:
                        ...
                        if not token:
                            ...
                            st.info("Set environment variable: export FYERS_ACCESS_TOKEN='your_token'")
                        else:
                            ...
                            for res, name in [("5", "5-Min"), ("15", "15-Min"), ("60", "1-Hour")]:
                                ...
                                if df is not None and len(df) > 10:
                                    try:
                                        ...
                                        last_time = df["Time"].iloc[-1].strftime("%d-%b-%Y %H:%M:%S")
...
export FYERS_ACCESS_TOKEN="your_token"
        ...
        if os.environ.get("FYERS_ACCESS_TOKEN"):
            st.success("✅ FYERS_ACCESS_TOKEN is set")
        else:
            st.warning("⚠️ FYERS_ACCESS_TOKEN not set")
        
        if os.environ.get("FYERS_APP_ID"):
            st.success("✅ FYERS_APP_ID is set")
        else:
    ...
    with tabs[2]:
        ...
        with col1: st.metric("NSE Symbols Loaded", len(st.session_state.all_symbols))
        with col2: st.metric("F&O Stocks Loaded", len(st.session_state.fo_symbols))
        ...
        ✅ Signal time fixed (correct candle close time)
