...
class OrderBlockSignal:
    ...
    def generate_chart(self, df: pd.DataFrame, folder: str = None) -> Optional[str]:
        ...
        try:
            ...
            ob_rect = patches.Rectangle((len(df) - 5, self.order_block_low), 5, self.order_block_high - self.order_block_low, linewidth=2, edgecolor='blue' if self.signal_type == "BUY" else 'red', facecolor='blue' if self.signal_type == "BUY" else 'red', alpha=0.2, label=f"Order Block Zone")
...
class SignalDeduplicator:
    ...
    def cleanup_old_signals(self):
        ...
        expired_keys = [key for key, timestamp in self.signals.items() if (current_time - timestamp).total_seconds() / 3600 > SIGNAL_15M_DUPLICATE_PREVENTION_HOURS]
    ...
    try:
        ...
        if bullish_ob == "🟢 Bullish OB":
            if not signal_deduplicator.is_duplicate(symbol, signal_time_str):
                ...
                signal = OrderBlockSignal(symbol=stock_ticker, signal_type="BUY", signal_date=signal_date_str, signal_time=signal_time_str, timeframe="15 Minutes", order_block_high=ob_high, order_block_low=ob_low, entry_price=entry, stop_loss=sl, target_1=t1, target_2=t2, current_price=last_close, volume_avg_20=vol_avg20, volume_current=last_volume, rsi=rsi, macd_bullish=macd_bullish, signal_strength=ob_strength, risk_reward_ratio=rr_ratio)
        ...
        if bearish_ob == "🔴 Bearish OB":
            if not signal_deduplicator.is_duplicate(symbol, signal_time_str):
                ...
                reward = abs(t1 - entry)
                rr_ratio = round(reward / risk, 2) if risk > 0 else 0.0
                signal = OrderBlockSignal(symbol=stock_ticker, signal_type="SELL", signal_date=signal_date_str, signal_time=signal_time_str, timeframe="15 Minutes", order_block_high=ob_high, order_block_low=ob_low, entry_price=entry, stop_loss=sl, target_1=t1, target_2=t2, current_price=last_close, volume_avg_20=vol_avg20, volume_current=last_volume, rsi=rsi, macd_bullish=macd_bullish, signal_strength=ob_strength, risk_reward_ratio=rr_ratio)
                signals.append(signal)
                signal_deduplicator.record_signal(symbol, signal_time_str)
                signal.save_as_txt()
                signal.save_as_csv()
...
def _analyse(symbol: str, df: pd.DataFrame, nifty_close: Optional[pd.Series], enable_xgboost: bool) -> dict:
    ...
    return {"Signal Date": signal_date_str, "Signal Time": signal_time_str, "Stock": stock_ticker, "LTP": round(last_close, 2), "Gap %": gap_str, "Target": target, "Stoploss": stoploss, "SMC Structure": smc_structure, "CISD": cisd_signal, "Bullish Order Block": bullish_ob, "Bearish Order Block": bearish_ob, "Order Block Zone": ob_zone, "Order Block Strength": ob_strength, "XGBoost Trend": xgb_trend, "XGBoost Confidence (%)": xgb_confidence, "News": news, "Alerts": alerts, "Signal Strength": signal_strength, "Entry Confirmation": entry_confirmation, "Signal Reason": signal_reason, "Trade Quality": trade_quality, "Trade Decision": trade_decision, "MTF Trend": mtf_trend, "AI Trend": ai_trend, "AI Confidence (%)": ai_confidence, "RS vs NIFTY": rs_label, "Support": round(float(support), 2) if pd.notna(support) else None, "Resistance": round(float(resistance), 2) if pd.notna(resistance) else None, "52W High": round(float(h52w), 2) if pd.notna(h52w) else None, "52W Low": round(float(l52w), 2) if pd.notna(l52w) else None, "52W Status": status_52w, "RSI": rsi_val, "Breakout Status": breakout, "MACD Signal": macd_signal_str, "Supertrend": supertrend_label, "VWAP": vwap_val, "Chart Pattern": chart_pattern, "RVOL": rvol_display, "AI Score": ai_score, "Final Signal": final_signal, "Smart Money": "🏦 Institutional" if ai_score > 70 else "⚖️ Neutral" if ai_score > 45 else "🔻 Distribution", "Signal": "🟢 BUY" if ai_score > 65 else "🔴 SELL" if ai_score < 40 else "🟡 HOLD", "_ATR14": round(float(atr14), 2) if pd.notna(atr14) else round(last_close * 0.01, 2), "_RVOL_RAW": rvol_raw, "_Is_High_Quality": is_high_quality, "_Quality_Count": quality_count}
...
def _color_code(val):
    if isinstance(val, str):
        if any(x in val for x in ["Strong Buy", "BUY", "Institutional", "🟢", "🔵", "Buy", "BOS 📈", "CHOCH 🐂", "Bullish", "Aligned Bullish", "Outperform", "Near High", "Bullish Engulfing", "Hammer", "Higher Highs", "📈", "Up"]):
            return "color: green; font-weight: bold;"
        if any(x in val for x in ["Strong Sell", "SELL", "Sell", "Distribution", "🔴", "🟠", "BOS 📉", "CHOCH 🐻", "Bearish", "Aligned Bearish", "Underperform", "Near Low", "Bearish Engulfing", "Shooting Star", "Lower Highs", "📉", "Down"]):
...
_SIGNAL_FILL_RULES = [("STRONG BUY", "006100", "FFFFFF", True), ("STRONG SELL", "9C0006", "FFFFFF", True), ("WATCHLIST", "FFA500", "000000", True), ("BUY", "92D050", "000000", True), ("SELL", "FF0000", "FFFFFF", True), ("WAIT", "FFFF00", "000000", True), ("HOLD", "FFFF00", "000000", True), ("CISD UP", "92D050", "000000", True), ("CISD DOWN", "FF0000", "FFFFFF", True)]
...
def _fetch_intraday_cisd_signal(fyers, symbol: str, resolution: str, timeframe_label: str):
    ...
    try:
        ...
        reason = (f"{timeframe_label} CISD {'bullish' if is_up else 'bearish'} shift confirmed on candle close (RSI {rsi_val}, RVOL {_format_rvol_display(rvol_raw)})")
        row = {"Signal Date": signal_date_str, "Signal Time": signal_time_str, "Timeframe": timeframe_label, "Stock": stock_ticker, "Signal": signal_label, "Entry": entry, "Stoploss": sl, "Target": target, "Confidence %": confidence, "AI Score": ai_score, "News": calculate_news(stock_ticker, 0.0, rvol_raw, "📈 Bullish" if is_up else "📉 Bearish"), "Reason": reason}
        return row, None
    except (KeyError, IndexError, TypeError, ValueError, ZeroDivisionError, AttributeError) as e:
        return None, f"{symbol}: analysis error ({type(e).__name__})"
...
def _fetch_fo_cisd_signal(fyers, symbol: str):
    ...
    try:
        ...
        row = {"Signal Date": signal_date_str, "Signal Time": signal_time_str, "Symbol": stock_ticker, "LTP": round(last_close, 2), "Signal": signal_label, "Entry": entry, "SL": sl, "Target": target, "Confidence": confidence, "Trend": supertrend_label, "Volume": int(last_volume), "RVOL": _format_rvol_display(rvol_raw), "News": calculate_news(stock_ticker, gap_pct, rvol_raw, "📈 Bullish" if is_bull else "📉 Bearish")}
        return row, None
    except (KeyError, IndexError, TypeError, ValueError, ZeroDivisionError, AttributeError) as e:
        return None, f"{symbol}: analysis error ({type(e).__name__})"
...
def _fetch_golden_death_cross_signal(fyers, symbol: str):
    ...
    try:
        ...
        stock_ticker = symbol.replace("NSE:", "").replace("-EQ", "")
        signal_date_str, signal_time_str = _candle_signal_timestamp(df, is_daily=True)
        row = {"Signal Date": signal_date_str, "Signal Time": signal_time_str, "Stock": stock_ticker, "Cross Type": cross_type, "Signal": signal_label, "Entry": entry, "Stoploss": sl, "Target 1": t1, "Target 2": t2, "Target 3": t3, "Holding Period (Days)": holding_days, "Estimated Exit Date": exit_date, "Trend Strength": trend_strength, "Confidence %": confidence, "AI Score": ai_score, "News": calculate_news(stock_ticker, 0.0, rvol_raw, "📈 Bullish" if is_bull else "📉 Bearish")}
        return row, None
    except (KeyError, IndexError, TypeError, ValueError, ZeroDivisionError, AttributeError) as e:
        return None, f"{symbol}: analysis error ({type(e).__name__})"
...
def _fetch_premarket_signal(fyers, symbol: str):
    ...
    try:
        ...
        stock_ticker = symbol.replace("NSE:", "").replace("-EQ", "")
        signal_date_str, signal_time_str = _candle_signal_timestamp(df, is_daily=True)
        row = {"Signal Date": signal_date_str, "Signal Time": signal_time_str, "Stock": stock_ticker, "Buy Volume": int(buy_volume), "Sell Volume": int(sell_volume), "Buy/Sell Ratio": buy_sell_ratio, "Gap %": f"{gap_pct:.2f}%", "RVOL": _format_rvol_display(rvol_raw), "AI Score": ai_score, "Expected Opening Trend": expected_trend, "News": calculate_news(stock_ticker, gap_pct, rvol_raw, "📈 Bullish" if bullish_votes >= 3 else ("📉 Bearish" if bearish_votes >= 3 else "NO"))}
        return row, None
    except (KeyError, IndexError, TypeError, ValueError, ZeroDivisionError, AttributeError) as e:
        return None, f"{symbol}: analysis error ({type(e).__name__})"
