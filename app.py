...
# SIDEBAR
# =====================================================
st.sidebar.image("https://assets.fyers.in/images/logo.svg", width=180)
...
# =====================================================
try:
    if menu == "🏠 Dashboard": show_dashboard(fyers)
    elif menu == "📈 Market": show_market(fyers)
    elif menu == "💼 Portfolio": show_portfolio(fyers)
    elif menu == "📋 Orders": show_orders(fyers)
    elif menu == "⚙️ Option Chain": show_option_chain(fyers)
    elif menu == "🤖 Scanner": show_scanner(fyers)
    elif menu == "📊 Charts": show_charts(fyers)
    elif menu == "💹 Trading": show_trading(fyers)
    elif menu == "👤 Profile": show_profile(fyers)
    elif menu == "⚙️ Settings": show_settings()
except Exception as e:
    st.error("Error Loading Module")
    st.exception(e)  # ఇక్కడ బ్రాకెట్ క్లోజ్ చేయబడింది
...
try:
    ...
    if profile.get("s") == "ok":
        ...
        st.sidebar.success(f"👤 {data.get('display_name', 'FYERS User')}")
        st.sidebar.caption(f"Client ID : {data.get('fy_id', '-')}")
except:
...
st.divider()
st.markdown("<center><h5>🚀 FYERS Trading Dashboard</h5></center>", unsafe_allow_html=True)
